...
export function DashboardContent() {
  ...
  // Sync with searchParams / URL hash
  React.useEffect(() => {
    ...
    const viewParam = searchParams.get("tab") as DashboardView;
  ...
  const handleViewChange = (view: DashboardView) => {
    setActiveView(view);
    if (typeof window !== "undefined") {
      router.push(`/dashboard?tab=${view}#${view}`, { scroll: false });
  ...
  const handleStartLesson = (lessonId: string) => {
    ...
    if (typeof window !== "undefined") {
      router.push(`/dashboard?tab=workspace&lesson=${encodeURIComponent(lessonId)}#workspace`, { scroll: false });
      window.scrollTo({ top: 0, behavior: "smooth" });
    }
  };
